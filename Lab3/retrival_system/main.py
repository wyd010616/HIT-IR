...
from PyQt5.QtWidgets import QApplication, QMainWindow, QMessageBox, QHeaderView, QAbstractItemView
...
class MainWindow(QMainWindow, login_Form):
    ...
    def login(self):
        ...
        if user != '游客' and pwd != password:
            QMessageBox.warning(self, '警告', '密码错误，登录失败！')
        else:
...
class SearchWindow(QMainWindow, search_Form):
    ...
    def __init__(self):
        ...
        self.setupUi(self)
        self.retriver = joblib.load('../model/retriver_model.pkl')
        # 按钮触发绑定
...
#             if row <= num:
    ...
    def web_search(self):
        ...
        # '网址','文章标题','相关文档列表'
            ...
            top5 = self.retriver.search_web(query)
            if self.level == 1: # 老师
                for i in range(5):
                    data = self.retriver.web_dt.iloc[top5[i]]
                    self.paras.append(data['paragraphs'])
                    self.model.setItem(i, 0, QStandardItem(data['url']))
                    self.model.setItem(i, 1, QStandardItem(data['title']))
    ...
    def file_search(self):
        ...
        if self.level != 1:
            ...
            if query == '':
                ...
                top5 = self.retriver.search_file(query)
                for i in range(5):
                    data = self.retriver.file_dt.iloc[top5[i]]
